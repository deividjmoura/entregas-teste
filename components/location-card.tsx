...
export function LocationCard({ local, contagem, temLinhaParada, children }: LocationCardProps) {
  ...
  const corChip = corParaLocal(local, 0.16, 62);
  return (
    <div
      className={`group relative flex flex-col overflow-hidden rounded-lg border bg-panel transition-all duration-150 ${temLinhaParada ? "border-rose-500/30 bg-rose-500/[0.01] dark:bg-rose-500/[0.02]" : "border-panel-border hover:border-zinc-700 dark:hover:border-zinc-800 shadow-premium"}`}
      style={!temLinhaParada ? { borderLeftWidth: 4, borderLeftColor: corForte } : undefined}
    >
      ...
      <button type="button" onClick={() => setAberto((v) => !v)} className="flex w-full items-start justify-between gap-3 px-4 pb-3 pt-4 text-left focus:outline-none">
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-2">
            <span
              className={`inline-flex items-center rounded px-1.5 py-0.5 font-display text-xs font-semibold uppercase tracking-wider ${temLinhaParada ? "text-rose-500" : "text-ink"}`}
              style={!temLinhaParada ? { backgroundColor: corChip } : {}}
            >
              {local}
            </span>
